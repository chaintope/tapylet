...
// c2/c3: Non-reissuable/NFT - two transactions
const issueNonReissuableToken = async (
  tpcUtxos: Utxo[],
  keyPair: tapyrus.ECPairInterface,
  publicKey: Buffer,
  p2cPublicKey: Buffer,
  ...
  // --- Transaction 2: Issue token from P2C output ---
  // OutPoint of the P2C output (first output of tx1)
  const outPoint = {
    txid: Buffer.from(tx1id, "hex").reverse(),
    index: 0,
  }

  // Derive colorId from OutPoint
